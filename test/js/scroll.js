$(document).ready(function(){
    ...
    let sec04_obj_top
    let sec05_obj_top
    let sec05_offset // sec05의 이동 값
    let scrolling // 현재 브라우저가 스크롤 되는 값
    let window_h // 브라우저의 높이 값
    let offset = 200 // 콘텐츠가 하단에서부터 offset 값 만큼 올라오면 애니메이션
    ...
    scroll_chk() //문서가 로딩되었을 때 1번실행
    $(window).resize(function(){
        scroll_chk() //브라우저의 사이즈가 변경되었을 때
    })
    $(window).scroll(function(){
        scroll_chk() //브라우저를 스크롤 할 때 마다 1번실행 
    })
})//$(document).ready
